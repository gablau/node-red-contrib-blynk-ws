module.exports = (RED) => {
  function BlynkInReadNode(n) {
    ...
    if (this.pin_all) this.connected_label = RED._('blynk-ws-in-read.status.connected-all');
    else this.connected_label = RED._('blynk-ws-in-read.status.connected-fixed') + this.pin;
    ...
    if (this.blynkClient) {
      ...
      this.blynkClient.on('disabled', () => {
        node.status({
          fill: 'red',
          shape: 'dot',
          text: 'blynk-ws-in-read.status.disabled',
        });
      });
    } else {
